...
class ReportGenerator:
    ...
    def _create_header(self, patient_info):
        ...
        # Hier greifen wir sicher auf die Keys zu
        p_id = patient_info.get('ID', '-')
        p_name = patient_info.get('Name', '-') # Leer, falls nicht vorhanden
    ...
    def _create_metric_row(self, label, metric_data, unit=""):
        ...
        """
        # Falls metric_data None oder leer ist, Fehler abfangen
        if not metric_data:
        ...
        if change_pct is not None and isinstance(change_pct, (int, float)):
            ...
            # Grün für positiv, Rot für negativ (optional)
    ...
    def build_report(self, metrics, plot_files):
        """
        NEUE SIGNATUR: Nur noch 2 Argumente!
        metrics: Das Dictionary, das ALLES enthält (Werte + 'meta' für Patientendaten)
        plot_files: Liste der Pfade zu den Bildern
        """
        ...
        # 1. Header (Patientendaten aus metrics['meta'] holen)
        patient_meta = metrics.get("meta", {})
        story.extend(self._create_header(patient_meta))

        # Links vs Rechts Layout
        left_content = []
        
        # A) KRAFT
        left_content.append(Paragraph("Kraftmessungen", self.styles['SectionHeader']))
        kraft_data = []
        ...
        # Daten einfüllen - Zugriff über Keys wie 'handkraft', 'sprung' etc.
        ...
        left_content.append(t_kraft)
        left_content.append(Spacer(1, 0.5*cm))

        # B) SPIRO
        left_content.append(Paragraph("Spiroergometrie", self.styles['SectionHeader']))
        spiro_data = []
        ...
        left_content.append(t_spiro)
        left_content.append(Spacer(1, 0.5*cm))

        # C) ANTHRO
        left_content.append(Paragraph("Anthropometrie", self.styles['SectionHeader']))
        anthro_data = []
        ...
        left_content.append(t_anthro)

        # RECHTS (Plots)
        right_content = []
        right_content.append(Paragraph("<b>Referenzwerte:</b>", self.styles['MetricLabel']))
        right_content.append(Paragraph("Perzentilkurven bieten eine visuelle Darstellung...", self.styles['ExplanationSmall']))
        right_content.append(Spacer(1, 0.2*cm))

        # Plots einfügen
        if not plot_files: # Falls Liste leer
            right_content.append(self._create_placeholder_img("Keine Plots vorhanden", 8*cm, 5*cm))
        else:
            for plot_path in plot_files:
                if os.path.exists(plot_path):
                    img = Image(plot_path, width=8*cm, height=5*cm)
                    right_content.append(img)
                    right_content.append(Spacer(1, 0.5*cm))
                else:
                    right_content.append(self._create_placeholder_img("Plot fehlt", 8*cm, 5*cm))
                    right_content.append(Spacer(1, 0.5*cm))

        # LAYOUT ZUSAMMENBAUEN
        main_table_data = [[left_content, right_content]]
        main_table = Table(main_table_data, colWidths=[10*cm, 8.5*cm])
        main_table.setStyle(TableStyle([('VALIGN', (0,0), (-1,-1), 'TOP'), ('LEFTPADDING', (0,0), (-1,-1), 0)]))
        story.append(main_table)
        
        # FOOTER
        story.append(Spacer(1, 1*cm))
        story.append(Paragraph("<b>Erklärung der Parameter:</b>", self.styles['MetricLabel']))
        story.append(Paragraph("Max. Handkraft: Beschreibt die maximale Kraft...", self.styles['ExplanationSmall']))
        story.append(Spacer(1, 0.5*cm))
        story.append(Paragraph("DECADE Projektleitung: Romina Ledergerber", self.styles['ExplanationSmall']))
